  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm">
        <h3 className="text-xl font-semibold text-emerald-900">{report.fileName}</h3>
        <p className="text-sm text-emerald-800/80">
          {report.fileType} • {(report.fileSize / 1024).toFixed(1)} KB •{" "}
          {new Date(report.uploadedAt).toLocaleString("en-IN")}
        </p>
        {report.description ? (
          <p className="mt-1 text-emerald-900">{report.description}</p>
        ) : null}
      ...
      {report.aiAnalysis ? (
        <div className="rounded-2xl border border-emerald-100 bg-white shadow-sm p-4">
          <div className="text-sm text-emerald-700/70">AI Analysis</div>
          <div className="mt-1 text-emerald-900 whitespace-pre-line">{report.aiAnalysis}</div>
        </div>
      ) : null}
    </div>
  );
